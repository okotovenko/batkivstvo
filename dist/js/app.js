(() => {
    ...
    function tabs() {
        ...
        function setTabsStatus(tabsBlock) {
            ...
            if (!isMobileView) tabsTitles.forEach(((tabsTitle, index) => {
                if (!tabsTitle.classList.contains("_tab-active")) if (tabsBlockAnimate) _slideUp(tabsContent[index], tabsBlockAnimate); else tabsContent[index].hidden = true;
            }));
        }
        function setTabsAction(e) {
            ...
            if (el.closest("[data-tabs-title]")) {
                ...
                if (!tabsBlock.querySelector("._slide")) {
                    if (isMobileView) tabTitle.classList.toggle("_tab-active"); else {
                        let tabActiveTitle = tabsBlock.querySelectorAll("[data-tabs-title]._tab-active");
                        tabActiveTitle = Array.from(tabActiveTitle).filter((item => item.closest("[data-tabs]") === tabsBlock));
                        if (tabTitle.classList.contains("_tab-active")) tabTitle.classList.remove("_tab-active"); else {
                            tabActiveTitle.forEach((title => title.classList.remove("_tab-active")));
    ...
    class Popup {
        ...
        eventsPopup() {
            ...
            toggles.forEach((toggle => {
                const toggleLeft = toggle.querySelector(".toggle__left");
                const toggleWidth = toggle.offsetWidth;
                const toggleLeftWidth = toggleLeft.offsetWidth;
            ...
            document.addEventListener("click", function(e) {
                const toggleLeft = document.querySelectorAll(".toggle__left");
                toggleLeft.forEach((element => {
 ...
 */
    ...
    class Draad {
        constructor(element, options = {}) {
            ...
            this.parent = () => {
                if (this.defaults.parentClass.length < 1) return document.getElementsByTagName("body")[0]; else return this.findAncestor(document.getElementsByClassName(element)[0], this.defaults.parentClass);
            };
        ...
        goThroughDots() {
            ...
            if (this.parent() === document.getElementsByTagName("body")[0]) for (let i = 0; i < dots.length; i++) {
            ...
            } else for (let i = 0; i < dots.length; i++) {
                let currentDot = document.getElementsByClassName(this.element)[i];
                let sampleDotOffsetX = offsetX || currentDot.offsetWidth / 2;
                let sampleDotOffsetY = offsetY || currentDot.offsetHeight / 2;
                let top = currentDot.getBoundingClientRect().top + scrollTop - clientTop;
        ...
        init() {
            ...
            let self = this;
            if (this.responsive === true) window.addEventListener("resize", (() => {
                this.resize(self);
            }));
        }
        resize(self) {
